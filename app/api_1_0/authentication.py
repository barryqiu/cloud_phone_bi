...
def verify_password(mobile_num_or_token, password):
    ...
    if request.endpoint == "api.new_user" or request.endpoint == "api.get_games":
        return True
    if password == '':
...
def before_request():
    # if register no verify
    if request.endpoint != "api.get_games" and request.endpoint != "api.new_user" and not g.current_user.confirmed:
